...
Key differences from OT English (eng.py), inherited from nt/arb.py's mechanics:
  BASE_BLOCK  — AVD's OT target TSV tokenizes on whitespace only; Arabic fuses
                ...
                check the lexeme: to Arabic singular
  ...
  NEGATION_BLOCK — Hebrew's two negators (lo indicative, al jussive/prohibitive)
                ...
                uses laysa (matches NT Arabic). Existential ein/ayin —
...
- Subject pronoun (pro-drop) — Arabic verb agreement recovers a dropped subject. No supplied Arabic pronoun expected when a waw-consecutive verb has no separate Hebrew pronoun token.
...
- Merged article — treated as PRIMARY, not secondary (see ARTICLES).
- Suffix marking the subject of an infinitive construct restructured as a finite clause — secondary to the finite verb (see PRONOMINAL SUFFIXES).
...
Hebrew direct object marker (אֶת/אֵת, pos=particle) → NEQ source (no Arabic equivalent particle).
Waw + Arabic asyndeton (rare) → waw word-part NEQ source.
Arabic conjunction with no Hebrew conjunction token → NEQ target.
Bare transliterated proper name with a Hebrew article token → NEQ source (Arabic never fuses al- onto a bare transliterated proper name).
...
Dual number: Arabic has a productive dual matching Hebrew's. Dual-to-dual is the majority pattern (knees, feet, "two years") but NOT automatic — real lexeme-specific exceptions collapse to Arabic singular (אַפָּיו "his nostrils" → أَنْفِهِ "his nose") or plural (דְּלָתַיִם "double doors" → مَصَارِيعُ; עַפְעַפַּי "my eyelids" → أَجْفَانِي)

## PASSIVE VOICE
Hebrew niphal/pual/hophal derived stems mark passive/reflexive/reciprocal/middle voice. Four coexisting Arabic strategies:
- True morphological passive (Form I/II/IV, yufʿal(u)/fuʿila) — the default expectation, most common overall.
  יֻסַּךְ (hophal, "is poured") → يُسْكَبُ: primary 1:1. נִתְּנָה (niphal, "was given") → أُعْطِيَتِ: primary 1:1.
- Derived-stem active-form verb (Form V/VII/VIII: tafaʿʿala/infaʿala/iftaʿala) — narrower, clusters around reciprocal/collective/self-affecting actions (subject's own action produces the result, not an external agent).
  יֵחָלֵק (niphal, "was divided") → ٱنْقَسَمَ (Form VII, "split itself"): primary 1:1. Same root/stem CAN take true passive instead for a genuine external-agent reading — check sense, not just the lexeme (e.g. נִקְרָא "called" → دُعِيَ for "your name is invoked over this city" vs. → تَسَمَّى for "he took on their name").
- Adjectival/stative (plain adjective or active participle) — for niphal forms marking an inherent STATE, not a passive EVENT.
  נִרְחָב (niphal, "wide") → وَاسِعٍ (plain adjective): primary 1:1. נִרְדָּם ("sound asleep") → نَائِمًا (plain active participle): primary 1:1.
- Active-voice conversion (agent promoted to subject) — rare.
...
## ARTICLES
Hebrew article (הַ/הָ/הֶ) appears as a separate word-part token (pos=particle). Never NEQ. TREATED AS PRIMARY, not secondary — Arabic's al- is a real, productive definite-article morpheme. Because al- is always fused, there is no Branch A/Branch B split — the article's primary link to the noun's record is automatic whenever the Hebrew article word-part is present.
  הָאָרֶץ → ٱلْأَرْضُ "the earth": source=[articlePart, אָרֶץ], target=["ٱلْأَرْضُ"] — both primary
Do not confuse the fused article with an explicit Hebrew demonstrative pronoun (הוּא/הִיא/זֶה/זֹאת/אֵלֶּה) following an articular noun ("the man, the that-one" = "that man"). Arabic's demonstratives (هَذَا/هَذِهِ/ذَلِكَ/تِلْكَ) are separate, non-fused words and align primary 1:1 to that demonstrative token specifically, never to the article.
...
## CONSTRUCT CHAINS
A construct chain expresses genitive by word order and construct form — no preposition token. Arabic's own idafa works identically: bare noun-noun juxtaposition, head noun first, no linking word needed.
  רוּחַ אֱלֹהִים → رُوحُ ٱللهِ "Spirit of God": source=[רוּחַ], target=["رُوحُ"] — primary 1:1; source=[אֱלֹהִים], target=["ٱللهِ"] — primary 1:1
...
Occupational participles after הָיָה/וַיְהִי (e.g. "was a shepherd/archer/armor-bearer"): bare construct is the MAJORITY even here — do not expect a supplied preposition by default. The one confirmed exception is a lexicalized idiom: אֲבִי + participle-phrase ("father of X-ers") consistently supplies لِ, secondary to the participle's record, while any inner construct chain inside the same phrase stays bare.
  ...
  אֲבִי יֹשֵׁב אֹהֶל → أَبًا لِسَاكِنِي ٱلْخِيَامِ "father of tent-dwellers" (لِ-idiom exception): source=[אֲבִי-construct], target=["أَبًا"] — primary 1:1; source=[יֹשֵׁב-construct], target=["لِسَاكِنِي"] — primary; (fused لِ- has no separate Hebrew correspondent, secondary); source=[אֹהֶל], target=["ٱلْخِيَامِ"] — primary 1:1 (inner chain, still bare)
...
- Waw word-part → fused وَ-/فَ-: primary. Asyndeton (rare) → NEQ source.
...
- אֲשֶׁר — Arabic's relative pronoun family (ٱلَّذِي/ٱلَّتِي/ٱلَّذِينَ, agreeing in gender/number): primary, when functioning as a true relative. As a complementizer introducing an oath/command complement clause, aligns to Arabic's complementizer أَنْ instead.
...
Pronominal suffixes are separate word-part tokens (pos=suffix). The deciding factor for primary-vs-secondary is the suffix's grammatical ROLE (subject vs. object/possessor), NOT which kind of word it attaches to.

- Object suffix or possessive suffix — PRIMARY, fused, regardless of host type (noun, preposition, finite verb, participle, or an infinitive construct's OBJECT). The suffix attaches onto whatever Arabic word carries it — both primary.
  ...
  כָּל מֹצְאִי → كُلُّ مَنْ وَجَدَنِي "whoever finds me" (participle restructured as finite verb, suffix fuses onto it): both primary
  לְהַכְעִיסוֹ → لِإِغَاظَتِهِ "to provoke him" (infinitive construct's OBJECT suffix, fused onto the resulting masdar — contrast SUBJECT case below): both primary
  אֵלָיו → إِلَيْهِ "to him" (preposition): both primary

- Suffix marking the SUBJECT of an infinitive construct — SECONDARY, absorbed. The one exception, only for SUBJECT-marking (contrast OBJECT above). When restructured as an Arabic finite clause (the default — see INFINITIVAL CONSTRUCTIONS), Arabic verb agreement already encodes the subject, so the suffix has no separate word to attach to.
...
Check: is the suffix the infinitive's SUBJECT ("his becoming king") or its OBJECT ("his provoking someone")? Finite-clause restructuring absorbs subject suffixes; a masdar rendering keeps object suffixes fused as primary.\
"""

NEGATION_BLOCK = """\
## NEGATION

Hebrew's two morphologically distinct negators (לֹא indicative, אַל jussive/prohibitive) both converge on Arabic لَا.
...
- אַל (jussive/prohibitive) → لَا + jussive/imperative verb: primary 1:1. Same particle as indicative לֹא;only the verb's mood morphology distinguishes the prohibition.
  אַל תַּבִּיט → لَا تَنْظُرْ "do not look": source=[alId], target=["لَا"] — primary 1:1
- Nominal/copular negation → لَيْسَ (conjugated negator-verb, agreeing with its subject).
  לֹא אִישׁ אֵל → لَيْسَ ٱللهُ إِنْسَانًا "God is not a man": source=[loId], target=["لَيْسَ"] — primary 1:1
- אֵין/אַיִן (existential) — three coexisting strategies, none a single fixed phrase:
  ...
  (c) لَمْ يَكُنْ (negated finite "to be") — real but narrower than (a)/(b);clusters in narrative prose with a definite concrete-noun subject.
...
לֹא...עוֹד ("no longer") is DISCONTINUOUS. The negator fuses onto the verb; the "again"-adverb (بَعْدُ/أَيْضًا) trails at the very end of the clause, usually separated by the verb's object or other material — do not expect them adjacent.
...
Substantive/attributive articular participles split three ways by referent type:

- Genuine Arabic participle (ism al-fāʿil, often lexicalized as an agent-noun) — majority, for attributive modification of a specific referent, occupational titles, epithets.
...
- كُلُّ مَنْ / كُلُّ مَا + finite verb — the dominant strategy specifically for the casuistic legal formula כָּל הַ+participle ("whoever/whatever does X, Y"). مَنْ for personal referents (majority — ritual-purity law, wisdom generalizations); مَا for impersonal. A genuine participle/relative clause is used instead when the SAME syntax describes a specific group, not a hypothetical legal class.
...
- Predicative (participle as main clause predicate): finite-verb conversion is the MAJORITY strategy, not a bare participle predicate. Bare-participle predication is real but minority, clustering around postural/stative verbs (standing, sitting) — Arabic's active participle naturally expresses a resultant state, favoring a finite verb for dynamic actions.
...
- Formulaic quotative לֵאמֹר ("saying," introducing direct speech): a fixed, extremely stable rendering قَائِلًا.
...
Classical Arabic has no true infinitive usable this way. For the temporal/purpose use, Hebrew's בְּ/כְ + infinitive construct ("when/as X happened") renders as an ordinary finite clause introduced by حِينَ/لَمَّا ("when"), never a nonfinite form. The infinitive's own subject suffix, if present, is absorbed into the finite verb's agreement morphology (see PRONOMINAL SUFFIXES).
...
Three coexisting strategies,none a single default:
- Purposive لِ/لِكَيْ + subjunctive verb — majority, for genuine purpose clauses.
  לְהַבְדִּיל → لِتَفْصِلَ "to separate": primary
- أَنْ + subjunctive complement clause — for infinitive complements of volition/refusal/permission verbs.
...
The same בִּלְתִּי+infinitive ("so as not to X") can render three different ways (أَنْ لَا+subjunctive; a bare relative clause ٱلَّذِي لَا يَنْفَعُ) — treat as translator-variable.

### Infinitive absolute (cognate emphasis)
Cognate accusative and complete non-marking are roughly EQUALLY common (~half each), including within AVD itself — not a stable AVD-vs-ONAV split.
- Cognate accusative (masdar as accusative object of the matching finite verb) — structurally unique to this Semitic pair among supported languages.
...
A string of bare infinitive absolutes as imperatival commands (no matching finite verb, e.g. Isaiah 21:5) always renders as ordinary finite imperative/imperfect verbs.\
"""
